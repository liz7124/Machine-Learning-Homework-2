...
class CNN:
    ...
    def set_kernel_initializer(self, _initialization):
        if (_initialization == 'he_normal'):
            self.kernel_initializer = tf.initializers.he_normal()
        elif (_initialization == 'he_uniform'):
            self.kernel_initializer = tf.initializers.he_uniform()
        elif (_initialization == 'xavier_normal'):
            self.kernel_initializer = tf.contrib.layers.xavier_initializer(uniform=False)
        elif (_initialization == 'xavier_uniform'):
...
graph = plot.Plot()
kernel_initializer = ['he_normal','he_uniform','xavier_normal','xavier_uniform']
...
for i in range(4):
    ...
    cnn.load_data()
    cnn.normalization()
    cnn.set_kernel_initializer(kernel_initializer[i])
    ...
    training_title_list.append(kernel_initializer[i])

"""
cnn.set_kernel_initializer('he_uniform')
model = cnn.data_construct()
result = cnn.training(model, EPOCHS, BATCH_SIZE)
cnn.prediction(model)
training_result_list.append(result)
training_title_list.append(cnn.initilization)

cnn.set_kernel_initializer('xavier_normal')
model = cnn.data_construct()
result = cnn.training(model, EPOCHS, BATCH_SIZE)
cnn.prediction(model)
training_result_list.append(result)
...
cnn.set_kernel_initializer('xavier_unifrom')
model = cnn.data_construct()
result = cnn.training(model, EPOCHS, BATCH_SIZE)
cnn.prediction(model)
training_result_list.append(result)
